...
//! usuario** que arranca el supervisor (`bongocat --supervise`) con la sesión
...
const UNIT_NAME: &str = "bongocat.service";

/// `~/.config/systemd/user/bongocat.service` (respeta `$XDG_CONFIG_HOME`).
...
fn unit_contents(exec: &str) -> String {
    format!(
        "[Unit]\n\
         Description=Bongo Cat — overlay animado para Wayland\n\
         Documentation=https://github.com/athomo001/wayland-bongocat-reload\n\
...
fn systemctl(args: &[&str]) {
    match Command::new("systemctl").arg("--user").args(args).status() {
        Ok(s) if s.success() => {}
        Ok(s) => eprintln!(
            "bongocat: `systemctl --user {}` salió con {s}",
            args.join(" ")
        ),
        Err(e) => eprintln!("bongocat: no se pudo ejecutar systemctl: {e}"),
    }
}
...
pub fn install() -> ExitCode {
    ...
    if let Some(dir) = path.parent() {
        if let Err(e) = std::fs::create_dir_all(dir) {
            eprintln!("bongocat: no se pudo crear {}: {e}", dir.display());
            return ExitCode::from(1);
        }
    }
    if let Err(e) = std::fs::write(&path, unit_contents(&exec.to_string_lossy())) {
        eprintln!("bongocat: no se pudo escribir {}: {e}", path.display());
        return ExitCode::from(1);
    }
    println!("bongocat: unidad instalada en {}", path.display());
...
pub fn uninstall() -> ExitCode {
    ...
    match std::fs::remove_file(&path) {
        Ok(()) => println!("bongocat: unidad borrada ({})", path.display()),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            println!("bongocat: no había unidad instalada");
        }
        Err(e) => {
            eprintln!("bongocat: no se pudo borrar {}: {e}", path.display());
            return ExitCode::from(1);
        }
    }
    systemctl(&["daemon-reload"]);
