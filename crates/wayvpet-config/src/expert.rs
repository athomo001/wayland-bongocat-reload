//! "Modo experto": editar los ficheros `.ini` a mano (`wayvpet.conf`, y el
...
impl RawFile {
    ...
    pub fn save(&mut self, instance: Option<&str>, theme_name: &str) {
        // Un parseo de aviso: si es el `wayvpet.conf`, marca las líneas que el
        // parser no entendería (no impide guardar: es modo experto).
        if !self.is_theme {
            let (_, warns) = wayvpet_common::config::parse_ini(&self.text);
            if !warns.is_empty() {
...
/// Ficheros editables: siempre `wayvpet.conf`; y si el tema activo resuelve a
...
pub fn gather(model: &Model) -> Vec<RawFile> {
    ...
    if let Some(p) = model.path.clone().or_else(io::resolve_config_path_real) {
        if let Some(f) = RawFile::load("wayvpet.conf", p, false) {
            out.push(f);
        }
    }
    ...
    roots
        ...
        .map(|r| {
            if r.ends_with("themes") {
                ...
                r.join("wayvpet/themes").join(name)
            }
        })
        .find(|p| p.is_dir())
}
