...
/// Lee el PID guardado, comprueba que sea un `wayvpet` vivo y le manda
...
pub fn run(target: Option<&str>) -> Outcome {
    ...
    eprintln!("wayvpet: parando la instancia PID {pid}");
    if kill_process(target, Signal::Term).is_err() {
        return Outcome::NotRunning;
    }
    for _ in 0..50 {
        if !proc_alive(pid) {
            eprintln!("wayvpet: detenido");
            return Outcome::Stopped;
        }
        sleep(Duration::from_millis(100));
    }
    eprintln!("wayvpet: no responde, SIGKILL");
    let _ = kill_process(target, Signal::Kill);
    Outcome::Stopped
}

/// PID del fichero si apunta a un proceso vivo llamado `wayvpet`. Limpia el
...
fn read_running_pid(path: &std::path::Path) -> Option<i32> {
    ...
    if !proc_alive(pid) || comm(pid).as_deref() != Some("wayvpet") {
