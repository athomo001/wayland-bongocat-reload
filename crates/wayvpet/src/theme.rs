...
//! (parseo del `theme.ini`, aspecto) vive en `wayvpet_common::theme`; aquí
...
//! wayvpet nunca se queda sin gato.

use std::io;
use std::path::{Path, PathBuf};

use wayvpet_common::sheet::{parse_sheet_ini, SheetTheme};
use wayvpet_common::theme::{
    format_supported, parse_theme_ini, ThemeMeta, DEFAULT_FRAME_FILES, THEME_FORMAT_SUPPORTED,
};
use wayvpet_common::vpet::{parse_vpet_ini, VpetConfig};
...
impl LoadedTheme {
    ...
    pub fn cat_align(&self) -> Option<wayvpet_common::config::Align> {
        self.vpet.cat_align
    }
...
pub fn search_dirs() -> Vec<PathBuf> {
    ...
    if let Some(d) = home_share {
        dirs.push(d.join("wayvpet/themes"));
    }
...
pub fn resolve(spec: &str) -> Option<LoadedTheme> {
    ...
    let dir = if spec.contains('/') {
        ...
        if !p.is_dir() {
            eprintln!("wayvpet: tema: '{spec}' no es un directorio; uso el clásico");
            return None;
        }
        p
    } else {
        if spec.contains("..") || spec.contains('\0') {
            eprintln!("wayvpet: tema inválido: '{spec}'");
            return None;
        }
        match search_dirs()
            .into_iter()
            ...
            .find(|d| d.is_dir())
        {
            Some(d) => d,
            None => {
                eprintln!("wayvpet: tema '{spec}' no encontrado; uso el clásico");
                return None;
            }
        }
    };
...
fn load_dir(dir: &Path) -> Option<LoadedTheme> {
    ...
    if !format_supported(&meta) {
        eprintln!(
            "wayvpet: tema {}: theme_format {} > {} soportado; uso el clásico",
            dir.display(),
            meta.theme_format,
    ...
    for (i, name) in meta.frame_files.iter().enumerate() {
        ...
        let md = match std::fs::symlink_metadata(&path) {
            ...
            Ok(_) => {
                eprintln!(
                    "wayvpet: tema: {} no es un fichero regular ≤ 2 MiB",
                    path.display()
                );
                return None;
            }
            Err(e) => {
                eprintln!("wayvpet: tema: falta {} ({e})", path.display());
                return None;
            }
        };
        let _ = md;
        ...
        if resvg::usvg::Tree::from_data(&bytes, &resvg::usvg::Options::default()).is_err() {
            eprintln!(
                "wayvpet: tema: {} no parsea como SVG; uso el clásico",
                path.display()
            );
            return None;
        }
    ...
    eprintln!("wayvpet: tema '{label}' cargado de {}", dir.display());
    Some(LoadedTheme {
        meta,
        dir: dir.to_path_buf(),
...
fn load_sheet(dir: &Path, ini: &str, meta: ThemeMeta, vpet: VpetConfig) -> Option<LoadedTheme> {
    ...
    if sheet.frame_w == 0 || sheet.frame_h == 0 {
        eprintln!(
            "wayvpet: tema {}: falta frame_w/frame_h; uso el clásico",
            dir.display()
        );
        return None;
    }
    if sheet.states.is_empty() {
        eprintln!(
            "wayvpet: tema {}: ningún state_<n>_row/_frames utilizable; uso el clásico",
            dir.display()
        );
        return None;
    }
    ...
    if files.is_empty() {
        eprintln!(
            "wayvpet: tema {}: ni 'sheet =' ni 'sheet_<estado> ='; uso el clásico",
            dir.display()
        );
        return None;
    }
    ...
    for name in files {
        ...
        if name.contains("..") || name.contains(['/', '\\', '\0']) {
            eprintln!(
                "wayvpet: tema {}: 'sheet = {name}' debe ser un nombre de fichero simple",
                dir.display()
            );
            return None;
        }
        ...
        match std::fs::symlink_metadata(&path) {
            ...
            Ok(_) => {
                eprintln!(
                    "wayvpet: tema: {} no es un fichero regular ≤ 16 MiB",
                    path.display()
                );
                return None;
            }
        ...
        let mut fs = match png_decode::decode_frames(&bytes) {
            Ok(v) => v,
            Err(e) => {
                eprintln!("wayvpet: tema: {}: {e}; uso el clásico", path.display());
                return None;
            }
        };
        let src = if fs.len() == 1 {
            ...
            if png.w < need_w || png.h < need_h {
                eprintln!(
                    "wayvpet: tema {}: la hoja {name} ({}×{}) es menor que la rejilla \
            ...
            if fs[0].w != sheet.frame_w || fs[0].h != sheet.frame_h {
                eprintln!(
                    "wayvpet: tema {}: el APNG {name} es {}×{}, no {}×{} (frame_w/frame_h); \
...
/// Primer directorio de temas **escribible** (`$XDG_DATA_HOME/wayvpet/themes`).
pub fn user_themes_dir() -> PathBuf {
    ...
    base.join("wayvpet/themes")
}

/// `wayvpet theme new NOMBRE`: crea la carpeta del tema con los 5 SVG del
...
/// `wayvpet theme check NOMBRE|RUTA`: intenta cargarlo y reporta. `true` = OK.
#[must_use]
    ...
    match resolve(spec) {
        Some(t) => {
            ...
            if let ThemeArt::Sheet(s) = &t.art {
                if s.sheet.input_model == wayvpet_common::sheet::InputModel::Activity {
                    println!(
