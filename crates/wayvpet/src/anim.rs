...
use wayvpet_common::sheet::{self, Anchor, SheetTheme};
...
/// SVG embebidos, en el orden de `wayvpet_common::paw::FRAME_*`:
...
/// Los usa `wayvpet theme new` como plantilla de partida.
#[must_use]
...
";
    ...
    fn cache_de_sprite_sheet_recorta_escala_y_premultiplica() {
        let sheet = wayvpet_common::sheet::parse_sheet_ini(SHEET_INI);
    ...
    fn rasterize_sheet_monta_frames_y_arranca_en_idle() {
        ...
        let no_driv = wayvpet_common::sheet::parse_sheet_ini(
...
";
        let sheet = wayvpet_common::sheet::parse_sheet_ini(ini);
