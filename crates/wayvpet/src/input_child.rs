...
use wayvpet_common::config::MousePaw;
use wayvpet_common::mouse::mouse_paw_bit;
use wayvpet_common::paw::paw_for_keycode;
...
pub fn run(
    ...
    let open_all = |paths: &[String]| -> Vec<(Device, String)> {
        paths
            ...
            .filter_map(|p| match Device::open(p) {
                Ok(d) => Some((d, p.clone())),
                Err(e) => {
                    eprintln!("wayvpet[input]: no se pudo abrir {p}: {e}");
                    None
                }
            })
            .collect()
    ...
    if kbds.is_empty() && mice.is_empty() {
        eprintln!("wayvpet[input]: ningún dispositivo; el hijo termina");
        exit(0);
    }
...
fn keyboard_thread(mut dev: Device, path: &str, write_fd: RawFd) {
    loop {
        let batch = match dev.fetch_events() {
            Ok(b) => b,
            Err(e) => {
                eprintln!("wayvpet[input]: {path} dejó de leer: {e}");
                return;
            }
        };
        for ev in batch {
            ...
            if ev.event_type() == EventType::KEY && ev.value() == 1 {
                ...
                if !send_bit(
                    write_fd,
                    paw_for_keycode(i32::from(ev.code())) | wayvpet_common::paw::PAW_KEY,
                ) {
...
fn mouse_thread(mut dev: Device, path: &str, write_fd: RawFd, paw: MousePaw, _interval_ms: u64) {
    ...
    let mut last_gaze = wayvpet_common::mouse::GazeDirection::Center;
    ...
    loop {
        ...
        let new_gaze = wayvpet_common::mouse::gaze_direction_from_delta(gaze_x, gaze_y, 10);
        if new_gaze != last_gaze || moved {
            last_gaze = new_gaze;
            if !send_bit(write_fd, wayvpet_common::mouse::gaze_to_byte(new_gaze)) {
                return;
            }
        }
...
fn harden() {
    ...
    unsafe {
        if libc::prctl(libc::PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) != 0 {
            eprintln!("wayvpet[input]: PR_SET_NO_NEW_PRIVS falló; seccomp no se aplicará");
        }
...
fn install_seccomp() {
    ...
    let arch = match TargetArch::try_from(std::env::consts::ARCH) {
        ...
        Err(_) => {
            eprintln!(
                "wayvpet[input]: seccomp no disponible en '{}'; sigo sin filtro de syscalls",
                std::env::consts::ARCH
            );
            return;
        }
    ...
    let filter = match SeccompFilter::new(
        ...
        Ok(f) => f,
        Err(e) => {
            eprintln!("wayvpet[input]: no se pudo construir el filtro seccomp: {e}");
            return;
        }
    };
    ...
    let prog: seccompiler::BpfProgram = match filter.try_into() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("wayvpet[input]: no se pudo compilar el filtro seccomp: {e}");
            return;
        }
    };
    ...
    match seccompiler::apply_filter_all_threads(&prog) {
        Ok(()) => eprintln!(
            "wayvpet[input]: seccomp activo ({} syscalls permitidas)",
            allow.len()
        ),
        Err(e) => eprintln!("wayvpet[input]: no se pudo aplicar seccomp: {e}"),
    }
}
...
pub fn parent_bridge<R: Read>(mut pipe: R, tx: calloop::channel::Sender<u8>) {
    ...
    loop {
        match pipe.read(&mut buf) {
            Ok(0) => {
                eprintln!("wayvpet: el proceso lector de input terminó");
                return;
            }
            Ok(n) => {
                for &b in &buf[..n] {
            ...
            Err(e) => {
                eprintln!("wayvpet: fallo leyendo del proceso lector: {e}");
                return;
            }
        }
    }
