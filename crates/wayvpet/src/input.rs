...
fn resolve(configured: &[String], pred: fn(&Device) -> bool, rol: &str) -> Vec<String> {
    ...
    if detected.is_empty() {
        eprintln!("wayvpet: no se detectó ningún {rol} en /dev/input (¿grupo 'input'?)");
    } else {
        eprintln!("wayvpet: {rol}s detectados:");
        for (p, n) in &detected {
    ...
    for path in configured {
        match Device::open(path) {
            ...
            Ok(_) => eprintln!("wayvpet: {path} no parece un {rol}; se ignora"),
            Err(e) => eprintln!("wayvpet: no se pudo abrir {path}: {e} (¿grupo 'input'?)"),
        }
    }
    ...
    if chosen.is_empty() {
        chosen = detected
            ...
            .collect();
        if !chosen.is_empty() {
            eprintln!("wayvpet: uso los {rol}s detectados automáticamente");
        }
    }
    chosen
}
...
pub fn start(cfg: &Config) -> io::Result<Isolated> {
    ...
    let mice = if cfg.enable_mouse {
        ...
        eprintln!("wayvpet: enable_mouse=0; el ratón se ignora");
        Vec::new()
    };
...
fn reap_in_background(pid: libc::pid_t) {
    let _ = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            eprintln!("wayvpet: el proceso lector de input ({pid}) terminó");
        });
}
