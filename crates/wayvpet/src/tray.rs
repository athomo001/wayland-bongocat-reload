...
//! usuario sigue teniendo `wayvpetctl` (`show`/`hide`/`reload`/`restart`/`stop`).
...
pub enum TrayCommand {
    ...
    RestartOverlays,
    /// Abrir la ventana gráfica `wayvpet-config` (spec 0007). Nunca una
...
impl Tray {
    ...
    pub fn tooltip(&self, monitors: usize) -> String {
        match self.status {
            TrayStatus::Normal => format!("wayvpet — {monitors} monitor(es)"),
            TrayStatus::Hidden => "wayvpet — oculto".to_string(),
            TrayStatus::Error => "wayvpet — overlay caído (clic para reiniciar)".to_string(),
        }
    }
}
...
impl ksni::Tray for SniTray {
    ...
    fn icon_name(&self) -> String {
        // Vacío a propósito: no hay ningún icono llamado "wayvpet" instalado
...
const CONFIG_GUI_BIN: &str = "wayvpet-config";

/// ¿Está `wayvpet-config` en el `PATH` como ejecutable? El ítem "Configurar…"
...
/// "Configurar…": lanza la ventana gráfica `wayvpet-config`. Lista fija, sin
...
/// quede un zombi** — `wayvpet` no tiene un `SIGCHLD` que lo recoja.
pub fn launch_config() {
    match std::process::Command::new(CONFIG_GUI_BIN).spawn() {
        ...
        Err(e) => eprintln!("wayvpet: no se pudo abrir {CONFIG_GUI_BIN}: {e}"),
    }
}
...
const ICON_PNG: &[u8] = include_bytes!("../../../assets/tray/wayvpet-icon.png");
...
fn embedded_icon_base() -> (Vec<u8>, u32) {
    match crate::png_decode::decode_frames(ICON_PNG) {
        ...
        Ok(_) => {
            eprintln!("wayvpet: icono del tray: el PNG embebido no es cuadrado; sin icono");
            (Vec::new(), 0)
        }
        Err(e) => {
            eprintln!("wayvpet: icono del tray: {e}; sin icono");
            (Vec::new(), 0)
        }
    }
}
