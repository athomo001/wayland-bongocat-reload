//! Parseo y validación de `wayvpet.conf` (formato INI propio).
//!
...
//! rutas XDG y el escaneo de `/dev/input` viven en los binarios (`wayvpet` /
//! `wayvpetctl`), no aquí.
//!
...
/// Claves canónicas del `wayvpet.conf` — **una por ajuste**, en la grafía que
...
/// Configuración completa de wayvpet. `Config::default()` reproduce
...
pub struct Config {
    ...
    /// `wayvpet/src/wl.rs::State::next_wake` reprograma el tick a un instante
...
/// El `wayvpet.conf.example` que `install.sh` copia como config inicial: son
...
pub const EXAMPLE_INI: &str = include_str!("../../../../wayvpet.conf.example");

/// Configuración **de fábrica**: la del `wayvpet.conf.example` shipped. La usa
...
impl Config {
    ...
    /// `wayvpet --print-default-config` y como base de un fichero nuevo. El
