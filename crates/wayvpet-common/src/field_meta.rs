...
//! los widgets de `wayvpet-config` (spec 0007) y la validación previa de
//! `wayvpetctl set` / IPC `SET`.
//!
...
pub const FIELDS: &[FieldMeta] = &[
    ...
    boolean(
        ...
        Section::Advanced,
        "Permite que wayvpetctl y la ventana hablen con la instancia en marcha. Recomendado dejarlo activo.",
    ),
    boolean(
        "enable_tray",
        "Icono en la bandeja",
...
/// Validación **previa** para `wayvpetctl set` / IPC `SET` / la ventana: primero
